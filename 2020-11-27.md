...
* Scaled Dot Product Attention
  ...
  * python-dictを使ったアナロジー説明
    ...
    * query&keyが同一文章となる比較(self attention)
    ...
    # Kとdot productをする関数は、Qを受け取ってdictを返す関数とみなす。
    trK_dot_prod_(q) = {"1番目+今日":"qと<50番目+今日>の内積値", 
                        "2番目+は":"qと<16番目+は>の内積値", 
                        "3番目+雨":"qと<1番目+雨>の内積値", 
                        "4番目+が":"qと<100番目+が>の内積値", 
                        "5番目+上がり":"qと<14番目+上がり>の内積値",
                        ...}
    # keyが無限に(あらゆる埋め込みベクトルの個数くらい)あるとする。
    ```
    * 文章の一部と単語の比較
...
### Ablation Study
* 比較項目
  * (w/ w/o aux) ファインチューン時点で言語モデルのロス関数を含めて最適化するか否か
  * (w/o pretraining) 事前学習するか否か
