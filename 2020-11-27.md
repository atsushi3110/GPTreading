...
* 例を観察してみる：
  ...
  * 問題意識：
    * 相対位置ver : Xという記号のI個前にYという記号がある確率を計算してみてはどうだろうか？
    * 絶対位置ver : 絶対位置IにあるXという記号と絶対位置JにあるYという記号が出現する確率を計算してみてはどうだろうか？
...
* 問題意識をTransformerの文脈で考える。
  ...
  * ２：I個前の位置という情報はどのように表現しているだろうか？相対位置か絶対位置か？
    * Positional Encoding：絶対位置が特徴量エンジニアリング的な計算で与える（pythonのdictは順序がないが、この特徴量でtuple&listみたいな順序を持たせられる。）
...
* Scaled Dot Product Attention
  ...
  * queryとして、絶対位置Iと単語Xが足されたベクトル
  * query
    * python-dictでのアナロジー
    * dictに類似したデータ構造を与える。
    ```python
    query = "14番目+リンゴ" 
    # 実際は位置の特徴量ベクトルと「リンゴ」の埋め込みベクトルの足し算
    K_dot_product = {"50番目+リンゴ":v0, "16番目+東京":v1, "100番目+魚":v3, "14番目+みかん":v4} 
    # Kとdoct productをする関数＝dictとみなす。
    K_dot_product[query] = {"16番目+リンゴ":内積値0, "16番目+東京":内積値1, "10番目+リンゴ":内積値3, "14番目+みかん":内積値4} # QとKの内積をとること＝辞書へのアクセス
    ```
    * 上記のアナロジーの例では、Key Errorは起きない
    * softmaxをとると、{"50番目+リンゴ":0.4, "16番目+東京":0.05, "100番目+魚":0.05, "14番目+みかん":0.4}のようになることが望まれる。 
  * softmaxのtoken内容の分布ベクトルがどの内容へ「読み出す」かを決定する。
  * 分布ベクトルとValueのベクトルの次元ごとの掛け算(position wise multiply)してValueの内容を読み出す
  * softmaxを取る前に割り算しているのは、数値計算した勾配が小さくなりすぎることを防ぐため。
