...
* Scaled Dot Product Attention
  ...
  * queryとして、絶対位置Iと単語Xが足されたベクトルqを入れると、qと
  * python-dictを使ったアナロジー説明
    ...
    * 上記の`q`のリスト（＝文章全体）をQとして、softmaxをとるとのような出力が望まれる。 
    ```python
     Q = ["1番目+パリ", "2番目+で", "3番目+リンゴ",...]
     softmax(trK_dot_prod_(Q)) = [ {"50番目+リンゴ":0.0, 
                                    "16番目+東京":0.2, 
                                    "3番目+パリ":0.6, 
                                    "3番目+で":0.0, 
                                    "14番目+みかん":0.0, 
  ...
  * softmaxの分布ベクトルが「読み出す」内容＆位置を決定する。
    * 内容と位置が一致していればいるほど1.0に近くなるはず。
  * 分布ベクトルとVの行列掛け算でVの内容を読み出す
    * Vの１番目の列ベクトルから東京やパリなどが読み出される
    * 
