...
### 導入
* 例を観察してみる：
  * appleという記号が６番目にgreenという記号が３番目にある。（意味は無視！！）
  * greenという記号が５番目にeatingという記号が３番目にある。（意味は無視！！）
  * 問題意識：
...
* 問題意識をTransformerの文脈で考える。
  ...
  * ２：I番目やJに番目の位置という情報はどのように表現しているだろうか？
    * Positional Encoding：絶対位置を特徴量エンジニアリング的な計算で与える（pythonのdictは順序がないが、この特徴量でtuple&listみたいな順序を持たせられる。）
  * ３：統計的といっているが、単一の行列やNNの出力ではX,Y,I,Jなどの値が推論時に高いバイアスを持っていないだろうか？(under fittingが起こるか？)
...
* Scaled Dot Product Attention
  ...
  * python-dictでのアナロジー説明
    * dictに類似した仮想的なデータ構造を与える。（実現性は無視！）
    ```python
    Q = "14番目+リンゴ" 
    # 実際は位置の特徴量ベクトルと「リンゴ」の埋め込みベクトルの足し算
    K_dot_product = {"50番目+リンゴ":v0, "16番目+東京":v1, "100番目+魚":v3, "14番目+みかん":v4, ... } 
    # keyが無限（実数ベクトルの個数とおなじくらい）にあるとイメージする。
    # 気持ちとして、「...」は無限につづく。
    # Kとdoct productをする関数＝dictとみなす。
    K_dot_product[Q] = {"50番目+リンゴ":内積値0, 
                        "16番目+東京":内積値1, 
                        "100番目+魚":内積値3, 
                        "14番目+みかん":内積値4,
                        ...}
    # Qでアクセスすると唯一のvalueでなくdictが返ってくる。
    # dictは内積値
    # QとKの内積をとること＝辞書へのアクセス
    ```
  ...
  * 数学科の方への補足：
    * `K_dot_product`が線形汎関数を与えている気持ちです。
    * K, Qは双対な気持ちです。
