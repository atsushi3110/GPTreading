...
### 導入
* 例を観察してみる：
  * appleというtokenが６番目にgreenというtokenが３番目にある。（かかり受けという関係は無視）
  * greenというtokenが５番目にeatingというtokenが３番目にある。（述語と目的語という意味は無視）
  * 問題意識：
...
* 問題意識をTransformerの文脈で考える。
  ...
  * **３：複数の関係性（Bi-Gram＋かかりうけ＋述語目的語＋その他の人間の明示的知識に存在しない関係など）を同時に学習ができないだろうか？**
    * Multi Head Attention：上記１＆２からなる記憶モジュールを複数並列に推論した出力をcancatし線形レイヤーで統合する。

### Positional Encoding
  * 以下の式で行う。iは次元のindexで、posがtokenの位置
...
* Scaled Dot Product Attention
  ...
  * python-dictを使ったアナロジー説明
    ...
    K_dot_product(q) = {"50番目+リンゴ":"Qと<50番目+リンゴ>の内積値", 
                        ...
                        "1番目+パリ":"Qと<1番目+パリ>の内積値", 
                        "100番目+の":"Qと<100番目+の>の内積値", 
                        "14番目+みかん":"Qと<14番目+みかん>の内積値",
                        ...}
    # keyが(token位置の最大個数)x(tokenizerの語彙数)くらいあるとする。
    # よくある例：512*10kなど
    ```
    * 上記の`q`のリスト（＝文章全体）をQとして、softmaxをとるとのような出力が望まれる。 
    ```python
    Q = ["1番目+パリ", "2番目+の", "3番目+リンゴ",...]
     softmax(K_dot_product(Q)) = [ {"50番目+リンゴ":0.0, 
                                    "16番目+東京":0.4, 
                                    "1番目+パリ":0.4, 
                                    "100番目+の":0.0, 
                                    "14番目+みかん":0.0, 
     ...
     ```
    * word2vec同様に「みかん」は「りんご」とは内容が異なるが位置が同じなら置き換えても良さそうな文脈が統計的には多そうであるので
...
* seq2seqと同様にEncoderとDecoderを作成
* EncoderにはSource入力系列をそのまま入力
* ただし、DecoderにEncoderの出力と、Target入力系列をマスク記号で置換した入力の２つを入力する。
* マスク記号左から順番に真のTarget入力記号に置換する。
* 言語モデルの場合は、Source入力とTarget入力は同一
* 違いは、マスク記号の有無だけ。
